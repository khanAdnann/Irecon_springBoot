...
import com.irecon.innovation.entity.QsparcDomesticOfflinePresentmentRawData;
...
import com.irecon.innovation.repository.QsparcDomesticOfflinePrasentmentRawDataRepository;
import com.irecon.innovation.repository.QsparcDomesticPrasentmentRawDataRepository;
import com.irecon.innovation.repository.RupayDomesticAdjustmmentRawDataRepository;
import com.irecon.innovation.repository.RupayDomesticOfflinePrasentmentRawDataRepository;
...
@Service
public class QsparcDomesticPresentmentRawDataServiceImpl implements QsparcDomesticPresentmentRawDataService {

	private static final Logger logger = LoggerFactory.getLogger(QsparcDomesticPresentmentRawDataServiceImpl.class);

	@Autowired
	private QsparcDomesticPrasentmentRawDataRepository domesticPrasentmentRawDataRepository;
	@Autowired
	private QsparcDomesticOfflinePrasentmentRawDataRepository domesticOfflinePrasentmentRawDataRepository;

	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;

	@Override
	public boolean SaveQsparcDomesticPresetmentRawData(MultipartFile file, String fileDate, String Cycle) {
		logger.info("Save method SaveQsparcDomesticPresetmentRawData " + fileDate);


		List<QsparcDomesticPresentmentRawData> batchList = new ArrayList<>();
		List<QsparcDomesticOfflinePresentmentRawData> OPbatchList = new ArrayList<>();
		int batchSize = 1000;
		int Count = 0;
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if(file.getOriginalFilename().startsWith("OfflinePresentment")) {
					...
					QsparcDomesticOfflinePresentmentRawData p = new QsparcDomesticOfflinePresentmentRawData();
					...
					if (OPbatchList.size() == batchSize) {
						...
						domesticOfflinePrasentmentRawDataRepository.saveAll(OPbatchList);
					...
					QsparcDomesticPresentmentRawData p = new QsparcDomesticPresentmentRawData();
					p.setReportDate(clean(tempArr[0]));
					p.setPresentmentRaiseDate(clean(tempArr[1]));
					p.setPresentmentSettlementDate(clean(tempArr[2]));
					p.setFunctionCodeAndDescription(clean(tempArr[3]));
					...
					if (batchList.size() == batchSize) {
						...
						domesticPrasentmentRawDataRepository.saveAll(batchList);
			  ...
			  if (!OPbatchList.isEmpty()) {
				...
				if(file.getOriginalFilename().startsWith("OfflinePresentment")) {
			        ...
			        int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("QSPARC DOM OFFLINE PRESENTMENT CYCLE "+ Cycle, fileDate, file.getOriginalFilename(), String.valueOf(Count), "Y");
			        ...
			        int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("QSPARC DOM PRESENTMENT CYCLE "+ Cycle, fileDate, file.getOriginalFilename(), String.valueOf(Count), "Y");
			        logger.error(updateStatus > 0 ? "Upload Status updated" : "unable to update");
