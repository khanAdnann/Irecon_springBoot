...
import com.irecon.innovation.entity.JcbAcqRawData;
...
@Service
public class JcbAcqRawDataServiceImpl implements JcbAcqRawDataService{

	private static final Logger logger = LoggerFactory.getLogger(JcbAcqRawDataServiceImpl.class);
	
	@Autowired
	private JcbAcqRawDataRepository jcbAcqRawDataRepository;
	
	@Autowired
	 private CommonDataValidationRepository commonDataValidationRepository;
	
	@Override
    public boolean SaveJcbAcqRawData(MultipartFile file, String fileDate) {
        logger.info("Save method SaveNfsAcqRawData " + fileDate);


        Set<JcbAcqRawData> rawDataSet = new HashSet<>();
        ...
        try {
            ...
            while ((thisLine = reader.readLine()) != null) {
            	DataCount++;
                JcbAcqRawData data = new JcbAcqRawData();
            ...
            jcbAcqRawDataRepository.saveAll(rawDataSet);
            
            
            int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("JCB ACQUIRER", fileDate, file.getOriginalFilename(), String.valueOf(DataCount), "Y");
