...
import com.irecon.innovation.repository.IcdAdjustmentRawDataRepository;
...
@Service
public class IcdAdjustmentRawDataServiceImpl implements IcdAdjustmentRawDataService {
	private static final Logger logger = LoggerFactory.getLogger(IcdAdjustmentRawDataServiceImpl.class);

	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;

	@Autowired
	private IcdAdjustmentRawDataRepository icdAdjustmentRawDataRepository;

	public boolean SaveIcdAdjustmentFile(MultipartFile file, String fileDate) {
...
String cycle="";
		try {
			...
			List<IcdAdjustmentRawData> rowDataList = new ArrayList<>();
			for (Row r : sheet) {
				if (r.getRowNum() > 0) {
					IcdAdjustmentRawData data = new IcdAdjustmentRawData();
					int colIndex = 0;
			...
			icdAdjustmentRawDataRepository.saveAll(rowDataList);
			
			int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("ICD ADJUSTMENT",
	...
	private void setEntityValue(IcdAdjustmentRawData data, int index, String value) {
		switch (index) {
		case 0:
