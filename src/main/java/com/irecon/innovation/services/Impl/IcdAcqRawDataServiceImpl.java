...
@Service
public class IcdAcqRawDataServiceImpl implements IcdAcqRawDataService{
	private static final Logger logger = LoggerFactory.getLogger(IcdAcqRawDataServiceImpl.class);
	
	@Autowired
	private IcdAcqRawDataRepository icdAcqRawDataRepository;
	
	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;
	
	@Override
    public boolean SaveIcdAcqRawData(MultipartFile file, String fileDate) {
        logger.info("Save method SaveNfsAcqRawData " + fileDate);


        Set<IcdAcqRawData> rawDataSet = new HashSet<>();
        ...
        try {
            ...
            while ((thisLine = reader.readLine()) != null) {
            	DataCount++;
                IcdAcqRawData data = new IcdAcqRawData();
            ...
            icdAcqRawDataRepository.saveAll(rawDataSet);
            
            int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("ICD ACQUIRER", fileDate, file.getOriginalFilename(), String.valueOf(DataCount), "Y");
