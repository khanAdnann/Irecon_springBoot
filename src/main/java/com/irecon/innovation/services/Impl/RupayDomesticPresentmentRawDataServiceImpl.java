...
@Service
public class RupayDomesticPresentmentRawDataServiceImpl implements RupayDomesticPresentmentRawDataService {

	private static final Logger logger = LoggerFactory.getLogger(RupayDomesticPresentmentRawDataServiceImpl.class);

	@Autowired
	private RupayDomesticPrasentmentRawDataRepository domesticPrasentmentRawDataRepository;
	@Autowired
	private RupayDomesticOfflinePrasentmentRawDataRepository domesticOfflinePrasentmentRawDataRepository;

	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;

	@Override
	public boolean SaveRupayDomesticPresetmentRawData(MultipartFile file, String fileDate, String Cycle) {
		logger.info("Save method SaveRupayDomesticPrasetmentRawData " + fileDate);


		List<RupayDomesticPresentmentRawData> batchList = new ArrayList<>();
		List<RupayDomesticOfflinePresentmentRawData> OPbatchList = new ArrayList<>();
		int batchSize = 1000;
		int Count = 0;
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if(file.getOriginalFilename().startsWith("OfflinePresentment")) {
					...
					RupayDomesticOfflinePresentmentRawData p = new RupayDomesticOfflinePresentmentRawData();
					...
					RupayDomesticPresentmentRawData p = new RupayDomesticPresentmentRawData();
					p.setReportDate(clean(tempArr[0]));
					p.setPresentmentRaiseDate(clean(tempArr[1]));
					p.setPresentmentSettlementDate(clean(tempArr[2]));
					p.setFunctionCodeAndDescription(clean(tempArr[3]));
			  ...
			  if (!OPbatchList.isEmpty()) {
				...
				if(file.getOriginalFilename().startsWith("OfflinePresentment")) {
			        ...
			        int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("RUPAY DOM OFFLINE PRESENTMENT CYCLE "+ Cycle, fileDate, file.getOriginalFilename(), String.valueOf(Count), "Y");
			        ...
			        int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("RUPAY DOM PRESENTMENT CYCLE "+ Cycle, fileDate, file.getOriginalFilename(), String.valueOf(Count), "Y");
			        logger.error(updateStatus > 0 ? "Upload Status updated" : "unable to update");
