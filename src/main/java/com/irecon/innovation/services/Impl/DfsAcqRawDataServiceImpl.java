...
@Service
public class DfsAcqRawDataServiceImpl implements DfsAcqRawDataService{
	private static final Logger logger = LoggerFactory.getLogger(DfsAcqRawDataServiceImpl.class);

	@Autowired
	private DfsAcqRawDataRepository dfsAcqRawDataRepository;
	
	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;
	
	@Override
    public boolean SaveDfsAcqRawData(MultipartFile file, String fileDate) {
        logger.info("Save method SaveNfsAcqRawData " + fileDate);



        Set<DfsAcqRawData> rawDataSet = new HashSet<>();
        ...
        try {
            ...
            while ((thisLine = reader.readLine()) != null) {
            	DataCount++;
                DfsAcqRawData data = new DfsAcqRawData();
            ...
            dfsAcqRawDataRepository.saveAll(rawDataSet);
            
            int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("DFS ACQUIRER", fileDate, file.getOriginalFilename(), String.valueOf(DataCount), "Y");
