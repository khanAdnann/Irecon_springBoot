...
import com.irecon.innovation.entity.QsparcInternationalPresentmentRawData;
...
import com.irecon.innovation.repository.QsparcInternationalOfflinePrasentmentRawDataRepository;
import com.irecon.innovation.repository.QsparcInternationalPrasentmentRawDataRepository;
...
@Service
public class QsparcInternationalPresentmentRawDataServiceImpl implements QsparcInternationalPresentmentRawDataService {

	private static final Logger logger = LoggerFactory.getLogger(QsparcInternationalPresentmentRawDataServiceImpl.class);

	@Autowired
	private QsparcInternationalPrasentmentRawDataRepository qsparcPrasentmentRawDataRepository;
	@Autowired
	private QsparcInternationalOfflinePrasentmentRawDataRepository qsparcOfflinePrasentmentRawDataRepository;

	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;

	@Override
	public boolean SaveQsparcInernationalPresetmentRawData(MultipartFile file, String fileDate, String Cycle) {
		logger.info("Save method SaveQsparcInernationalPrasetmentRawData " + fileDate);

		List<QsparcInternationalPresentmentRawData> batchList = new ArrayList<>();
		List<QsparcInternationalOfflinePresentmentRawData> OPbatchList = new ArrayList<>();
		int batchSize = 1000;
		int Count = 0;
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (file.getOriginalFilename().startsWith("OfflinePresentment")) {
					...
					QsparcInternationalOfflinePresentmentRawData p = new QsparcInternationalOfflinePresentmentRawData();
					...
					if (OPbatchList.size() == batchSize) {
						...
						qsparcOfflinePrasentmentRawDataRepository.saveAll(OPbatchList);
					...
					QsparcInternationalPresentmentRawData p = new QsparcInternationalPresentmentRawData();
					p.setReportDate(clean(tempArr[0]));
					p.setPresentmentRaiseDate(clean(tempArr[1]));
					p.setPresentmentSettlementDate(clean(tempArr[2]));
					p.setFunctionCodeAndDescription(clean(tempArr[3]));
					...
					if (batchList.size() == batchSize) {
						...
						qsparcPrasentmentRawDataRepository.saveAll(batchList);
			...
			if (file.getOriginalFilename().startsWith("OfflinePresentment")) {
				...
				int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate(
						"QSPARC INT OFFLINE PRESENTMENT CYCLE " + Cycle, fileDate, file.getOriginalFilename(),
				...
				int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate(
						"QSPARC INT PRESENTMENT CYCLE " + Cycle, fileDate, file.getOriginalFilename(),
						String.valueOf(Count), "Y");
				logger.error(updateStatus > 0 ? "Upload Status updated" : "unable to update");
