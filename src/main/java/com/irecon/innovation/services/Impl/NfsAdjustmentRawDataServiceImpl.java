...
@Service
public class NfsAdjustmentRawDataServiceImpl implements NfsAdjustmentRawDataService {
	private static final Logger logger = LoggerFactory.getLogger(NfsAdjustmentRawDataServiceImpl.class);

	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;

	@Autowired
	private NfsAdjustmentRawDataRepository nfsAdjustmentRawDataRepository;

	public boolean SaveNfsAdjustmentFile(MultipartFile file, String fileDate) {
...
String cycle="";
		try {
			...
			List<NfsAdjustmentRawData> rowDataList = new ArrayList<>();
			for (Row r : sheet) {
				if (r.getRowNum() > 0) {
					NfsAdjustmentRawData data = new NfsAdjustmentRawData();
					int colIndex = 0;
			...
			nfsAdjustmentRawDataRepository.saveAll(rowDataList);

			int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("NFS ADJUSTMENT CYCLE "+cycle,
