...
@Service
public class RupayInternationlAdjustmentRawDataServiceImpl implements RupayInternationalAdjustmentRawDataService {

	private static final Logger logger = LoggerFactory.getLogger(RupayInternationlAdjustmentRawDataServiceImpl.class);

	@Autowired
	private RupayInternationalAdjustmmentRawDataRepository rupayAdjustmmentRawDataRepository;

	@Autowired
	private CommonDataValidationRepository commonDataValidationRepository;

	@Override
	public boolean SaveRupayInternationalAdjustmentRawData(MultipartFile file, String fileDate, String cycle) {
		logger.info("Save method SaveNfsIssRawData " + fileDate);



		 List<RupayInternationalAdjustmentRawData> batchList = new ArrayList<>();
		    int batchSize = 1000;
		    int Count = 0;
		    ...
		    try (CSVReader reader = new CSVReader(new InputStreamReader(file.getInputStream()))) {
		        ...
		        while ((fields = reader.readNext()) != null) {
		            ...
		            // Map to Entity
		            RupayInternationalAdjustmentRawData entity = new RupayInternationalAdjustmentRawData();
		            int i = 0;
	            ...
	            int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("RUPAY INT ADJUSTMENT CYCLE 1", fileDate, file.getOriginalFilename(), String.valueOf(Count), "Y");
