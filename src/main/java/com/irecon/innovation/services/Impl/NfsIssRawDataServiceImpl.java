...
@Service
public class NfsIssRawDataServiceImpl implements NfsIssRawDataService {
    private static final Logger logger = LoggerFactory.getLogger(NfsIssRawDataServiceImpl.class);

    @Autowired
    private NfsIssRawDataRepository nfsIssRawDataRepository;
    ...
    public boolean SaveNfsIssRawData(MultipartFile file, String fileDate) {
        ...
        Set<NfsIssRawData> rawDataSet = new HashSet<>();
        ...
        try {
            ...
            while ((thisLine = reader.readLine()) != null) {
            	DataCount++;
                NfsIssRawData data = new NfsIssRawData();
            ...
            nfsIssRawDataRepository.saveAll(rawDataSet);
            
            int updateStatus = commonDataValidationRepository.updateStatusByFileTypeOrFiledate("NFS ISSUER", fileDate, file.getOriginalFilename(), String.valueOf(DataCount), "Y");
